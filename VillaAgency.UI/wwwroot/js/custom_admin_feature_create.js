...
// Adding New FAQ (Adjusted so that error labels appear below the inputs)
function addFaq() {
    ...
    <div class="row g-2 mb-3 faq-item align-items-start">
        ...
        <div class="col-1 text-end pt-1">
            <button type="button" class="btn btn-sm text-danger p-0 border-0" onclick="removeItem(this, 'faq-item', 'FAQs')">✕</button>
        </div>
    </div>`;
...
// Adding New Detail (Adjusted so that error labels appear below the inputs)
function addDetail() {
    ...
    <div class="row g-2 mb-3 detail-item align-items-start">
        ...
        <div class="col-1 text-end pt-1">
            <button type="button" class="btn btn-sm text-danger p-0 border-0" onclick="removeItem(this, 'detail-item', 'FeatureDetails')">✕</button>
        </div>
    </div>`;
    container.insertAdjacentHTML("beforeend", html);
...
// Also synchronously updates 'data-valmsg-for' targets of error spans when indexes shift
function reindex(itemClass, rootName) {
    ...
    items.forEach((item, index) => {
        ...
        // Update both 'data-valmsg-for' values and name matches (if they exist)
